...
public class GenerationController : MonoBehaviour {
    ...
    private void GenerateWallMesh() {
        ...
        //getting error for some reason, make sure normals are as many as the vertices
    ...
    private int NeighbourCountVisited(Vector2Int pos, bool[,] visited) {
        int amount = 0;
    ...
    private void PlacePlayer() {
        if(spawnedPrefabs != null && spawnedPrefabs.Count > 0) {
            ...
            var player = FindObjectOfType<MovementController>().transform.gameObject;
    ...
    private void SpawnPrefabs() {
        ...
        for (int x = 0; x < grid.GetLength(0); x++) {
            for (int y = 0; y < grid.GetLength(1); y++) {
                if (grid[x, y].taken == 1 && grid[x, y].tileType == Tile.TileType.CORRIDOR && NeighbourCount(new Vector2Int(x, y)) == 1) {
                    Vector3 pos = grid[x, y].obj.transform.position;
                    GameObject newPrefab = Instantiate(prefabs[0]) as GameObject;
                    newPrefab.transform.position = pos;
                    newPrefab.transform.SetParent(prefabWrapper.transform, false);
    ...
    private void RotatePrefabs() {
        ...
        foreach(GameObject spawnedPrefab in spawnedPrefabs) {
            ...
            foreach (Vector3 direction in directions) {
                if (Physics.Raycast(spawnedPrefab.transform.position, direction, out RaycastHit hit, 1f)) {
    ...
    private void SpawnEnemies() {
        ...
        for (int i = 0; i < possibleSpawnPoints.Count; i++) {
            var dist = Vector2.Distance(possibleSpawnPoints[i], FindObjectOfType<MovementController>().transform.position);
            if(dist < 40f) {
        ...
        for (int i = 0; i < enemySpawnCount; i++) {
            var pos = possibleSpawnPoints[Random.Range(0, possibleSpawnPoints.Count)];
            var enemy = enemyPrefabs[Random.Range(0, enemyPrefabs.Length)];
            if (enemy != null && pos != null) {
                GameObject newEnemy = Instantiate(enemy) as GameObject;
                newEnemy.transform.position = pos;
                newEnemy.transform.eulerAngles = new Vector3(0, Random.Range(-180f, 180f), 0);
                newEnemy.transform.SetParent(enemyWrapper.transform, false);
    ...
    private void PrintGridInfo(Tile[,] g) {
        int wall = 0;
        int floor = 0;
        int filled = 0;
        int both = 0;
        for (int x = 0; x < g.GetLength(0); x++) {
            for (int y = 0; y < g.GetLength(1); y++) {
                if (g[x, y].taken == 1) floor++;
                if (g[x, y].taken == 0) wall++;
                if (g[x, y].filled) filled++;
                if (g[x, y].filled && g[x,y].taken == 1) both++;
            }
        }

        print("noTile: " + wall + ", tile: " + floor + ", filled: " + filled + ", both: " + both);
    }

    #region FLOOD_FILL
    int sections = 0;
    Tile[,] highestGrid = null;
    int highestSeciton = 0;
    ...
    private void FloodNode(int x, int y) {
        ...
        while (q.Count > 0) {
            var n = q.Pop();
            if (grid[n.x, n.y].taken == 1 && !grid[n.x, n.y].filled) {
        ...
        if(currentSection > highestSeciton) {
            highestSeciton = currentSection;
